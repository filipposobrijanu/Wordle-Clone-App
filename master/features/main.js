...
var BOXESBG = document.querySelectorAll('.box');

var KEYS = document.querySelectorAll('.box1');
...
const keys = ["Q","W","E","R","T","Y","U","I","O","P","A","S","D","F","G","H","J","K","L","Z","X","C","V","B","N","M","Backspace"];

var j = 0;

var randomword = Math.floor(Math.random() * 1000) + 1;

var word = words[randomword].toUpperCase();;
...
function End(){
    ...
    $(".popupcon").addClass("block");
    $(".blurcl").addClass("blurclass");
    $(".con1").addClass("blurclass");
...
function Check(j){
    ...
    for (i = j; i < j+5; i++) {
        if(guess[k]==wordarr[k]){

            BOXESBG[i].style.backgroundColor = "#14b524";
            letterscorr++;

                let u=i-1;
                for(let d = k-1; d>=0; d--){
                    if(guess[k]==guess[d] && guess[d]!==wordarr[d]){

                        BOXESBG[u].style.backgroundColor = "rgb(88, 88, 88)";
        ...
        else if(wordarr.includes(guess[k])){

            BOXESBG[i].style.backgroundColor = "rgb(221, 208, 25)";

            if(hasDuplicates(guess)==false){
            let u=i-1;
            for(let d = k-1; d>=0; d--){
                if(guess[k]==guess[d] && guess[d]==wordarr[d]){

                    BOXESBG[i].style.backgroundColor = "rgb(88, 88, 88)";
...
document.onkeydown = function(e) {
    ...
    if(key_press == 'Backspace' && guessi % 5 !=0 ){



        guessi--;
        metr--;
        $("#" + metr).animate({scale: "1"},200,"swing");
        BOXESBG[metr].style.backgroundColor = "transparent";
        BOXES[metr].innerHTML = "&nbsp;";
        guess.splice(metr,1);
        
    }
    const regex = /^[A-Za-z]$/
    if(regex.test(key_press)){

        guess[guessi] = key_press.toUpperCase();
        $("#" + metr).animate({scale: "0.9"},200,"swing");
        BOXES[metr].innerHTML = key_press.toUpperCase();
        guessi++;
        metr++;

    }
    if(guessi % 5 ==0 && regex.test(key_press)){

        let guessword1 = guess.toString();
        let guessword = guessword1;
        ...
        if(words.includes(guessword.toLowerCase())){
            ...
            $(".popupwrong").addClass("showp");
            
            setTimeout(function(){
                $(".popupwrong").removeClass("showp");
            }, 1000);
            for(let i1 = j; i1 < j+5; i1++){

                $("#" + i1).animate({scale: "1"},200,"swing");
                BOXES[i1].innerHTML = "&nbsp;";
                BOXESBG[i1].style.backgroundColor = "transparent";
...
function ClosePopup(){
    ...
    $(".popupcon").removeClass("block");
    $(".blurcl").removeClass("blurclass");
    $(".con1").removeClass("blurclass");
...
function clickk(id){
    ...
    if(id==26 && guessi % 5 !=0){
        ...
        if(id!=26){
        guess[guessi] = keys[id];
        $("#" + metr).animate({scale: "0.9"},200,"swing");
        BOXES[metr].innerHTML = keys[id];
        guessi++;
        metr++;
